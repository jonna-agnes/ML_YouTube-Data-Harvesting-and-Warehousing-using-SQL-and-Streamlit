...
# API version and service name
api_service_name = "youtube"
api_version = "v3"
api_key="AIzaSyB2x4ulIMkrGcj1DK-oS0HL198zos0w658"

youtube = googleapiclient.discovery.build(api_service_name, api_version, developerKey=api_key)

#Function to fetch the data from MYSQL Database
def fetch_data(query):
    mydb = mysql.connector.connect(host="localhost", user="root", password="12345", database="youtube_data")
...
def main():
    ...
    if Options == "Channels":
        ...
        # Button to store all fetched data into the database
        if st.button("Store Data in Database"):
            try:
                ...
                # Store channel data
                if 'channel_df' in locals() and not channel_df.empty:
                    for _, row in channel_df.iterrows():
                        cursor.execute("""
                            INSERT INTO channels (channel_name, channel_id, channel_des, channel_playid, channel_viewcount, channel_subcount)
                            VALUES (%s, %s, %s, %s, %s, %s)
                            ON DUPLICATE KEY UPDATE channel_name=VALUES(channel_name), channel_des=VALUES(channel_des),
                            channel_playid=VALUES(channel_playid), channel_viewcount=VALUES(channel_viewcount), channel_subcount=VALUES(channel_subcount)
                        """, tuple(row))

                # Store video data
                if 'video_df' in locals() and not video_df.empty:
                    for _, row in video_df.iterrows():
                        cursor.execute("""
                            INSERT INTO videos (Video_Id, Video_title, Video_Description, channel_id, video_Tags, Video_pubdate, Video_viewcount, 
                                Video_likecount, Video_favoritecount, Video_commentcount, Video_duration, Video_thumbnails, Video_caption)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                            ON DUPLICATE KEY UPDATE Video_title=VALUES(Video_title), Video_Description=VALUES(Video_Description),
                            video_Tags=VALUES(video_Tags), Video_pubdate=VALUES(Video_pubdate), Video_viewcount=VALUES(Video_viewcount),
                            Video_likecount=VALUES(Video_likecount), Video_commentcount=VALUES(Video_commentcount)
                        """, tuple(row))
                ...
                if 'comment_df' in locals() and not comment_df.empty:
                    for _, row in comment_df.iterrows():
                        cursor.execute("""
                            ...
                            VALUES (%s, %s, %s, %s, %s, %s)
                            ON DUPLICATE KEY UPDATE Comment_Text=VALUES(Comment_Text), Comment_Authorname=VALUES(Comment_Authorname),
